...
def book(request):
    if request.method=="POST":
        ...
        data=Rooms.objects.filter(is_available=True,location=location,no_of_days_advance__gte=no_of_days,start_date__lte=start_date)
...
def book_now(request,id):
    if request.session.get("username",None) and request.session.get("type",None)=='customer':
        if request.session.get("no_of_days",1):
            ...
            contact=data.manager.phone_no
            return render(request,"booking/book-now.html",{"no_of_days":no_of_days,"room_no":id,"data":data,"bill":bill,"contact":contact,"roomManager":roomManager,"start":start_date,"end":end_date})
        else:
            return redirect("index")
    else:
